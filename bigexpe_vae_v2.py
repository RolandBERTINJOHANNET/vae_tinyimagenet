...
class VanillaVAE(nn.Module):
    def __init__(self, in_channels: int, latent_dim: int, hidden_dims=None, beta=1.0, upsampling='bilinear', loss_type='lpips'):
        ...
        # Final convolution to adjust to the required output channels (e.g., 3 for RGB images), foll>
    ...
    def _deconv_block(self, in_channels, out_channels):
        if self.upsampling == 'convtranspose':
            ...
            return nn.Sequential(
                nn.Conv2d(in_channels, out_channels * 4, kernel_size=3, padding=1),  # Adjust channels for pixel shuffle
                nn.PixelShuffle(upscale_factor=2),
                nn.BatchNorm2d(out_channels),
                nn.LeakyReLU()
            )
        else:  # Standard upsampling methods like 'nearest', 'bilinear', etc.
            return nn.Sequential(
...
def train_vae(model, train_loader, val_loader, device, n_epochs=10, lr=1e-5):
